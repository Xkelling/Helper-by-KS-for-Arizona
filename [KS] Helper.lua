		...
		if settings.v then
			...
			if timer.v == true then
				...
				lua_thread.create(function ()
						...
						settings.v = false
						sampSetCursorMode(4)
					sampAddChatMessage(tag..'Нажмите {00BFFF}ПРОБЕЛ{FFFFFF} что-бы сохранить позицию', mc)
...
function imgui.Link(link,name,myfunc)
				...
				if imgui.IsItemHovered() then
						...
						imgui.GetWindowDrawList():AddLine(imgui.ImVec2(p.x, p.y + size.y), imgui.ImVec2(p.x + size.x, p.y + size.y), imgui.GetColorU32(imgui.GetStyle().Colors[imgui.Col.ComboBg]))
				else
...
function custom_color()
		...
		colors[clr.WindowBg] = ImVec4(0.136, 0.136, 0.136, 0.932)
		colors[clr.ChildWindowBg] = ImVec4(0.00, 0.00, 0.00, 0.00)
		colors[clr.PopupBg] = ImVec4(0.099, 0.099, 0.099, 0.999)
		colors[clr.Border] = ImVec4(0.86, 0.86, 0.86, 1.00)
		colors[clr.BorderShadow] = ImVec4(0.00, 0.00, 0.00, 0.00)
		colors[clr.FrameBg] = ImVec4(0.251, 0.251, 0.251, 1.000)
		...
		colors[clr.Button] = ImVec4(0.058, 0.617, 0.848, 1.000)
		colors[clr.ButtonHovered] = ImVec4(0.00, 0.46, 0.65, 1.00)
		colors[clr.ButtonActive] = ImVec4(0.000, 0.859, 1.000, 1.000)
